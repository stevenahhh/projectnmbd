...
## 아크 8 — 있는 줄 알았던 기능, 없는 줄 알았던 버그 (08-30)

- **튜토리얼이 두 번 떴다.** 원인은 로직이 아니라 배치였다. `<Tutorial>` 이 사이드바와 헤더 두 곳에 렌더돼 자동 실행이 각각 걸렸다. 인스턴스를 콘텐츠 영역 하나로 모으고, 탭별 `hanmok-tut:<탭>` 키로 "처음 보는 화면에서만 1회" 로 바꿨다.
- **초대 링크는 코드에만 있었다.** 팀 생성 직후 토스트는 "초대 링크를 만들어 팀원을 모으세요" 라고 안내하는데, `createInvite` 를 호출하는 화면이 없었다. 게다가 그 함수 자체가 `doc(db, 'invites')` — 컬렉션 경로에 문서 참조를 만들고 있어 불렸다면 던졌을 코드였다. 호출부가 없어서 아무도 몰랐다. 멤버 화면에 초대 카드(링크 생성 + 클립보드 복사)를 붙이고 경로를 고쳤다. **3~5인 팀플 앱에서 팀원을 부를 방법이 없었다는 뜻이다.**
- **회의 참석 이벤트가 새고 있었다.** 작성자가 참석자 체크박스로 팀원을 넣어도 남는 이벤트는 `meeting.create` 하나뿐이라, 회의 축 기여도는 각자 참석 체크를 눌러야만 쌓인다. 그 버튼이 상세 다이얼로그 맨 아래에만 있었다 — 카드에서 바로 누를 수 있게 올렸다.
- **AI 키가 없다는 사실을 숨기지 않았다.** 회의록 세 줄 요약은 `GEMINI_API_KEY` 가 있을 때만 실제로 요약하고, 없으면 503 과 함께 "직접 적어주세요" 로 물러난다. 키 없이 그럴듯한 요약을 만들어내는 대체 구현은 두지 않았다.
- **250줄 규칙으로 모듈을 갈랐다.** gantt-panel 455 → 205(좌표 계산은 `lib/timeline.ts`, 그림·창은 별도 컴포넌트), team-ops 282 → 도메인 6개, demo-dataset 525 → 픽스처 6개 + 빌더. 분리하면서 드래그 기간 계산·마크다운 파서·요약 파싱에 회귀 테스트를 붙여 28 → 55개가 됐다.
